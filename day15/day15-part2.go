...
func prepareRisks(risks [][]int) [][]int {
	newRisks := make([][]int, 5*len(risks))
	for row := 0; row < len(newRisks); row++ {
		newRisks[row] = make([]int, len(risks[0])*5)
	}

	for row := 0; row < len(risks); row++ {
		for col := 0; col < len(risks[0]); col++ {
			newRisks[row][col] = risks[row][col]
		}
	}

	for rowIdx := 0; rowIdx < 5*len(risks); rowIdx++ {
		for colIdx := len(risks[0]); colIdx < 5*len(risks[0]); colIdx++ {
			val := newRisks[rowIdx][colIdx-len(risks[0])] + 1
			if val > 9 {
				val = 1
			}
			newRisks[rowIdx][colIdx] = val
		}
	}

	for colIdx := 0; colIdx < 5*len(risks[0]); colIdx++ {
		for rowIdx := len(risks); rowIdx < 5*len(risks); rowIdx++ {
			val := newRisks[rowIdx-len(risks)][colIdx] + 1
			if val > 9 {
				val = 1
			}
			newRisks[rowIdx][colIdx] = val
